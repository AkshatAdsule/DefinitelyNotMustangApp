...
class TeamDataAnalyzer {
  ...
  static Map<String, double> getTeamTargetAverages(
    ...
    List<DocumentSnapshot> matches = getMatchDocs(teamNumber)
        .where((element) =>
            element.data()['hasAnalysis'] &&
            element.data()['target'] == targetType)
        .toList();
  ...
  static void _calcAverages() {
    ...
    for (int i = 0; i < _teams.length; i++) {
      ...
      averages[team.id] =
          _calcTeamAverages(team.id, team.data()['hasAnalysis']);
    }
  ...
  //   Reference ref = FirebaseStorage.instance.ref().child('db.json');
