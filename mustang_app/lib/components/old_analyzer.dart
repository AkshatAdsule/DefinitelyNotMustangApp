...
class _OldAnalyzerState extends State<OldAnalyzer> {
  ...
  String getReport() {
    ...
    double _percentTimeInDefense =
        100 * (_totalDefActionTime / _totalQualGameTime);
    ...
    return "Team: " +
        ...
        _totPtsPrev.toStringAsFixed(1).toString() +
        "\nPoints prevented/sec: " +
        _ptsPrevOverDefTime.toStringAsFixed(3) +
        "\n% time in defense: " +
        _percentTimeInDefense.toString() +
        "%, offense: " +
        _percentTimeInOffense.toString() +
        "%\nTotal reg fouls: " +
        _regFouls.round().toString() +
        ", tech: " +
        _techFouls.round().toString() +
        ", yellow: " +
        _yellowCards.round().toString() +
        ", red: " +
        _redCards.round().toString();
  }
  ...
  double calcPushPtsPrev() {
    ...
    for (var i = 0; i < _pushTime.length; i++) {
      ...
      var _actualDisplacement = calcDisplacement(
          _startColumn[i], _startRow[i], _endColumn[i], _endRow[i]);
  ...
  double calcDisplacement(
      double startColumn, double startRow, double endColumn, double endRow) {
    var _columnDisplacement =
        (endColumn - startColumn) * Constants.zoneSideLength;
